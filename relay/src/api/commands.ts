...
import { authenticateShortcutRequest, maybeTouchTokenLastUsed } from "../auth/shortcutAuth.js";
import { rateLimited } from "../utils/errors.js";

export async function registerCommandRoutes(app: FastifyInstance, ctx: AppContext): Promise<void> {
  app.post("/v1/devices/:deviceId/commands", async (request, reply) => {
    ...
    const result = await ctx.commands.submit(params.deviceId, request.body);
