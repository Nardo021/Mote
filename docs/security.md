...
- 客户端始终使用该 HTTPS 基址以及由其推导的 `wss://…/v1/ws/device` 和 `wss://…/v1/ws/pair`。Cloudflare Tunnel 不得改变这些安全 URL。
...
- 不要把 Mac 或快捷指令改成访问 `http://192.0.2.10:3000`。该地址只是 PVE 宿主机上 `cloudflared` 的源站。
- Mac 生产代码永不关闭 TLS 校验。
...
- 把 `192.0.2.10:3000` 发布到家庭 LAN 并不削弱 Relay 认证。Bearer、设备 WebSocket 认证、凭据角色分离、命令允许列表、速率限制、TTL、无命令队列和重复保护全部保留。健康检查可以保持当前的未认证设计。
